...
应用中许多功能页面需要多步操作才能到达（如：连接设备 → 选择设备 → 进入某个子页面 → 触发特定状态）。**当目标界面需要设备连接、特定数据状态、或多步导航才能到达时，必须主动向用户提问，请求用户协助操作到目标位置，然后再接手调试。**

```
适用场景:
- 需要真实设备连接才能显示的页面（设备详情、应用列表、文件管理等）
- 需要代理运行中才能产生数据的页面（网络请求列表、Mock 规则匹配测试等）
- 需要录制/会话进行中才能查看的页面（事件时间线、录制回放等）
- 需要特定操作序列才能复现的 Bug（拖拽上传、工作流执行等）
- 任何通过 Playwright 自动化无法轻易到达的深层状态
```

**正确做法**:
```
1. 向用户说明需要到达的目标界面和状态
   例: "我需要查看 Mock 规则编辑弹窗中条件编辑器的实际渲染效果，
        请帮我打开代理页面，创建或编辑一条 Mock 规则，然后告诉我可以了"

2. 用户操作完成后，用 Playwright snapshot/screenshot 接手查看当前状态
...
**错误做法**:
```
❌ 因为无法自动导航到深层页面，就跳过浏览器验证，直接凭代码猜测修改
❌ 尝试用 Playwright 模拟复杂的多步操作（连接设备等），浪费时间且大概率失败
❌ 假设"这个改动很简单，不需要验证"
```
...
✅ 正确: 询问用户 "请帮我连接设备并导航到 XX 页面" → 用户就位后 snapshot 接手调试
```
