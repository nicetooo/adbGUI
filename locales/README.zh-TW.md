...
### 📱 裝置管理
- **統一裝置列表**：無縫管理統一的實體和無線裝置（USB/Wi-Fi 自動合併）。
- **無線連接**：透過 IP/連接埠配對輕鬆連接，支援 mDNS。
- **裝置記錄**：快速存取之前連接過的離線裝置。
- **順序刷新**：更智能的輪詢機制，實現穩定、無閃爍的 UI。
...
- **JSON 格式化**：自動美化日誌中偵測到的 JSON 段落。

### 🌐 網路分析與代理 (MITM)
- **自動化抓包**：一鍵開啟 HTTP/HTTPS 代理伺服器，並透過 ADB 自動配置手機代理。
- **HTTPS 解密 (MITM)**：支援解密 SSL 流量，內建自動 CA 憑證生成與推送功能。
- **WebSocket 支援**：即時捕獲並查看 WebSocket 傳輸內容。
- **大數據處理**：支援捕獲高達 100MB 的完整數據包而不截斷，並擁有 5000 條日誌快取。
- **弱網模擬**：模擬真實網路環境，支援按裝置限制下行/上行頻寬以及設置人工延遲。
- **數值視覺化**：即時監控所選裝置的下行 (RX) / 上行 (TX) 速率。
...
- **裝置置頂**：將常用裝置置頂，方便在系統選單中快速操作。
- **功能直達**：從系統匣選單直接開啟置頂裝置的 Logcat、Shell 或檔案管理器。
